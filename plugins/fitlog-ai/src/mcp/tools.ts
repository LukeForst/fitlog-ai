...
export function registerFitlogTools(server: { registerTool: Function }, dependencies: FitlogToolDependencies): void {
  ...
  const ownerEmail = dependencies.ownerEmail;

  server.registerTool("log_workout", {
    title: "记录训练",
    description: "记录一次力量训练、动作和每组重量与次数。",
    inputSchema: workoutSchema
  }, (input: z.infer<z.ZodObject<typeof workoutSchema>>) => {
    const result = records.recordWorkout(ownerEmail, input);
    return {
  ...
  }, (input: z.infer<z.ZodObject<typeof mealSchema>>) => {
    const result = records.recordMeal(ownerEmail, input);
    return {
  ...
  }, (input: { date: string; weightKg: number; bodyFatPercent?: number }) => {
    const result = records.recordBodyMeasurement(ownerEmail, input);
    return {
  ...
  server.registerTool("get_dashboard", {
    ...
    _meta: dashboardMeta
  }, (input: { from: string; to: string; exerciseName?: string }) => {
    const result = dashboards.getDashboard(ownerEmail, { from: input.from, to: input.to }, input.exerciseName);
    return {
  ...
  }, (input: { from: string; to: string; kind: "workouts" | "meals" | "measurements" }) => {
    const result = input.kind === "workouts"
      ? dependencies.repository.listWorkouts(ownerEmail, input.from, input.to)
      : input.kind === "meals"
        ? dependencies.repository.listMeals(ownerEmail, input.from, input.to)
        : dependencies.repository.listMeasurements(ownerEmail, input.from, input.to);
    return {
  ...
  }, (input: { content: string; category?: string; isImportant: boolean }) => {
    const result = records.rememberFact(ownerEmail, input);
    return {
  ...
  server.registerTool("forget_fact", {
    ...
    const deleted = records.forgetFact(ownerEmail, input.memoryId);
    return {
